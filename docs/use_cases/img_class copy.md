...
이 문서에서는 Arm® *Ethos™ -U* NPU 이미지 분류 예제를 설정하고 실행하는 과정을 설명합니다.

이 사용 사례는 이미지 분류의 고전적인 컴퓨터 비전 문제를 해결한다. ML 샘플이 개발되었습니다
*ImageNet* 데이터 세트에서 학습된 *MobileNet v2* 모델을 사용합니다.
